...
def _parse_members_full_all(html_content: str, org_symbol: str, redacted_counter: int) -> List[Dict[str, Any]]:
    ...
    # Encontrar todas las posiciones de <li class="member-item
    li_positions = []
    for match in re.finditer(r'<li class="([^"]*)"[^>]*data-org-sid="([^"]*)" data-org-name="([^"]*)">', html_content):
        li_positions.append((match.group(1), match.group(2), match.group(3), match.start()))
    
    # Extraer HTML completo de cada li cortando hasta el siguiente
    all_matches = []
    for i, (li_classes, org_sid, org_name, start_pos) in enumerate(li_positions):
        # Fin: hasta el siguiente <li> o final del string
        end_pos = li_positions[i + 1][3] if i + 1 < len(li_positions) else len(html_content)
        
        # HTML completo del miembro
        member_html = html_content[start_pos:end_pos]
        all_matches.append((li_classes, org_sid, org_name, member_html))
